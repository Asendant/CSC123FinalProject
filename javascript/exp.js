...
let playerSpeedIncrement = 0.02;
let playerBaseDamage = 10;

const GROWTH_FACTOR_EXP = 1.5;
const GROWTH_FACTOR_DAMAGE = 0.05;
...
const levelUp = () => {
    ...
    player.bulletDamage = playerBaseDamage * Math.pow((1 + GROWTH_FACTOR_DAMAGE), currentPlayerLevel - 1);
...
function drawEXPBar() {
    ...
    textSize(16);
    text(`${Math.floor(displayedEXP)}/${Math.floor(expToNextLevel)}`, barX - 80, barY + barHeight / 2);

    textAlign(RIGHT, CENTER);
    text(`Level: ${currentPlayerLevel}`, barX + barWidth + 80, barY + barHeight / 2);
}
